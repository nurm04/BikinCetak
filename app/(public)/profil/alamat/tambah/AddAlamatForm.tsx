...
// ==========================================
// HELPER: NORMALISASI DATA API (STRICT TYPE)
// ==========================================
const normalizeData = (responsePayload: unknown): RegionOption[] => {
  ...
  return results.map((item) => {
    ...
    const itemId =
      item.id || item.province_id || item.city_id || item.subdistrict_id || item.district_id;

    const itemName =
      item.name || item.province || namaKota || item.district_name || item.subdistrict_name || item.label;
...
export default function AddAlamatForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await createAlamat(payload);
    ...
    if (res.success) {
      ...
      setPopup({
        isOpen: true,
        title: "Gagal",
        message: res.error || "Terjadi kesalahan saat menambahkan alamat.",
        type: "error",
      });
    }
  ...
  return (
    <>
      ...
      <form
        ...
        className="p-8 border shadow-sm card bg-base-100 border-base-content/5"
      >
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          <FormInput label="Label Alamat" name="label" value={form.label} onChange={handleChange} placeholder="Rumah / Kantor"/>
          ...
          <div className="md:col-span-2">
            <label className="flex items-center gap-3 w-fit cursor-pointer mt-2">
              <input
                type="checkbox"
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full h-14 px-10 mt-10 font-black tracking-widest uppercase shadow-lg md:w-auto rounded-xl btn btn-primary shadow-primary/20"
        >
          {loading ? (
